...
events
  ...
  title, venue,
  start_at, end_at,
  answer_deadline_at,         -- 出欠の回答期限
  capacity, price,
  audience,                   -- all 全員 / classes 指定クラス / selected 個別
  fee_collection,             -- none / on_site / with_tuition
                              -- / bank_transfer / other
  billing_month,              -- 合算する対象月（with_tuition のときだけ）
  fee_note,
  status,                     -- planned 予定 / held 開催済み / canceled 中止
  cancel_reason,
  is_public,                  -- 保護者のマイページに出すか
  invited_at,                 -- 案内メールを最後に送った日時
  description

event_target_classes
  id, organization_id, event_id, class_id

event_entries
  id, organization_id, event_id, student_id,
  status,                     -- invited 未回答 / entered 参加
                              -- / undecided 保留 / declined 不参加
                              -- / canceled 取り消し
  answered_at, answered_by,
  attendance,                 -- present / absent / unconfirmed
...
運営から出た指摘に合わせて直した（移行 039）。

**回答期限を持つ。** 衣装の発注や座席の都合があるので、いつまでに答えるのかが
決まっていないと運営が動けない。期限を過ぎたら保護者は答えられなくなり、以降の
変更は運営が聞き取って入れる。期限を入れなければ開催時刻が期限になる。

**「誰に案内するか」を持つ。** 毎回全員が出演するわけではない。全体・クラス指定・
個別の3つ。全体とクラス指定は `build_event_roster()` が名簿を作る。在籍の判定は
**開催日**で行う（いま在籍していても、開催日までに退会が決まっている生徒は
出演しない）。何度実行しても、すでに答えた返事は消えない。

**「未回答」はボタンとして出さない。** 参加にも不参加にも押していなければ未回答
だと見れば分かる。押せる選択肢として並べても、答えを消す操作が増えるだけになる。
状態としては残す（`invited`）。

**保護者の答えは参加・保留・不参加の3つ。** 発表会は数か月先で、仕事や家族の予定が
読めないことが多い。保留が無いと「とりあえず参加」を押されて、直前に減る。

**当日の出欠は出席と欠席だけ。** 学校ではないので、発表会に何分遅れたかを記録
しても使い道がない。

**出欠と参加可否は、あとから何度でも直せる。ただし直したことを残す**（2026-09-08
追加・移行 043）。当日は会場で手分けして記録するので、あとから直すのは正当な作業
であり、締めてしまうと記録漏れを入れられなくなる。止めるのではなく残す。

- 記録先は `audit_logs`（設計書 4.9）。同じ目的の表を2つ持たない
- **記録はトリガーで行う。** 入口が管理画面・講師の画面・保護者の回答
  （`answer_event_entry`）の3つあり、アプリ側に書くと1つ書き忘れても気づけない
- **操作した人の名前を、そのとき記録する**（`audit_logs.actor_name`）。あとで講師
  が退職して行が消えても「誰が変えたか」は残る。`deliveries.to_address` と同じ考え方。
  オーナー・スタッフには名前の置き場が無いので、ロールの名前（「オーナー」）で表す
- **参照範囲は分ける。** `audit_logs` には請求の訂正（金額）が入っており、そちらは
  講師に見せない（設計書 7章）。`target_type = 'event_entry'` の行だけを運営・講師
  にも開ける。記録するのは講師なので、自分の記録が誰にどう直されたかを見られない
  のは不便すぎる
- **名簿に出すのは「直した」ぶんだけ。** 未記録から出席になったのは記録であって
  訂正ではない。全部並べると訂正が埋もれる。保護者の最初の回答も同じ扱いで出さない

**参加費は「いくら」と「どうやって集めるか」を分けて持つ。** 当日徴収がもっとも
多いが、月謝に合算するスタジオも、事前振込のスタジオもある。案内メールにこの区別
がそのまま載る。

**月謝に合算するときは、何月分に載せるかをイベントごとに選ぶ**（2026-09-07 追加・
移行 040）。発表会は10月に案内して11月開催、集金は12月、という具合にずれる。開催月
から機械的に決めると実態と合わない。登録画面では開催月の前々月から3か月後までを
並べ、既定を翌月にしてある。

```
月次生成 app.generate_invoices_internal(org, 対象月)
  1. 月謝の請求を作る
  2. 兄弟割を適用する        ← 順位づけは月謝（subtotal）だけで行う
  3. app.apply_event_fees()  ← イベント参加費を明細として足す
  4. 合計・消費税を確定して issued へ
```

- **載せるのは「参加する」と答えた生徒だけ。** 保留・不参加・未回答・取消は請求
  しない
- **金額は `event_entries.amount`。** `events.price` から複写してあり、個別の免除
  ができる
- **同じ参加を二度請求しない。** `invoice_items.event_entry_id` に部分一意索引。
  請求を作り直しても明細が重複しない
- **兄弟割は月謝だけを対象にする。** 割引の順位づけを `invoices.subtotal`（月謝）
  で行い、参加費はそのあとに足す。参加費の多い子から割り引かれることがない
- **入金済みの請求には足さない。** 設計書 5.6 のとおり paid は編集不可。飛ばした
  件数を画面に返す
- **請求を先に作ってしまった場合**は、イベントの画面から `apply_event_fees()` を
  呼んで反映できる（合算月をあとから変えたときも同じ）
- `subtotal` は明細のプラス分の合計に揃える。`subtotal - discount_total = total`
  を崩さないため

**案内は「保護者に案内する」の1操作にまとめる。** 押すと公開され、同時に出欠の
お願いのメールが飛ぶ。公開とメールを分けると「公開したのに送り忘れた」「送ったのに
マイページに出ていない」が起きる。作った時点では公開しない（下書きの日時が保護者に
届かないようにするため）。すでに答えた保護者と、すでに送った宛先には飛ばない。

**衣装のサイズはイベントに紐づけない。** 発表会のたびに集めるのではなく、保護者が
マイページからいつでも登録・更新できる形にする（`student_measurements`。設計書
4.3 のとおり履歴として積む）。運営は必要になったときに最新値を見る。半年以上
更新が無ければ、保護者の画面から測り直しをお願いする。
...
**承認・見送りは、そのままメールで保護者へ届く**（2026-09-07 追加）。承認制に
した以上、申し込んだ側は確定したかどうかを知る手段がない。11章を参照。

**メールアドレスを必須にする理由は、連絡だけではない。** 体験のときに入れた
アドレスをそのまま入会申込でも使えば、`link_guardian_by_email()` がその
アドレスの保護者行に結びつけ、マイページがそのまま使える状態になる（4.6.1）。
電話番号では連絡はできても、ログインの鍵にはならない。
...
### 体験・見学の承認／見送り（2026-09-07 追加）

承認制にした（4.6.2）ので、申し込んだ保護者は結果を知る手段がない。承認でも
見送りでも必ずメールを送る。

```
オーナーが「承認」または「見送る」を押す
        → trials.status が pending から変わる
        → トリガー app.notify_trial_decision()
        → pg_net で Edge Function（send-trial-notice）を呼ぶ
        → Resend で送信 → deliveries に結果を残す
```

- **送信の引き金は、状態が `pending` から変わったときだけ。** トリガーの条件
  に `old.status = 'pending'` を入れてあるので、承認を二度押しても二通目は
  飛ばない。二重送信の防止をアプリ側の注意力に頼らない
- **確定の連絡には「いつ・どこで」を必ず入れる。** 日時だけでは、初めて来る
  人はスタジオの場所を知らない
- **見送りの連絡に理由は書かない。** 運営が理由をそのまま送りたい場面はまず
  なく、書ける欄も持っていない。問い合わせ先だけを添える
- **宛先は申込フォームのアドレス。** 体験の時点では保護者として登録されて
  いないので `deliveries.guardian_id` は入らない。`to_address` が残る
- **届いたかどうかを管理画面に出す。** 送信は非同期なので、承認した直後は
  「送信中」に見える。失敗に気づけないと、運営が保護者に何と答えればよいか
  分からなくなる

### 発表会・イベントの案内（2026-09-07 追加）

「保護者に案内する」を押すと、公開と同時に出欠のお願いが飛ぶ。

```
オーナーが「保護者に案内する」を押す
        → public.publish_event()
        → is_public = true / invited_at を記録
        → pg_net で Edge Function（send-event-invite）を呼ぶ
        → Resend で送信 → deliveries に結果を残す
```

- **宛先は世帯の請求先保護者。** 子ども1人につき1通（答えるのが子ども単位なので、
  きょうだいがいる家庭には2通届く）
- **まだ答えていない保護者にだけ送る。** 答えた人への催促にはしない
- **期限を必ず本文に入れる。** 期限が無いときは、その行そのものを出さない
- **参加費は「いくらを、どうやって集めるか」を1行で書く。** 金額だけでは、当日
  いくら持たせればよいのかが分からない
- **同じ宛先に二度送らない。** `deliveries` に `(event_entry_id, channel)` の
  部分一意索引（`status = 'sent'` のみ）
